...
impl VcpuFd {
    ...
    /// vcpu.set_sync_valid_reg(SyncReg::Register);
    /// vcpu.run();
    /// let guest_rax = unsafe { vcpu.sync_regs().regs.rax };
    /// ```
    /// # Safety
    ///
    /// This will copy out of bounds memory if the allocation for `kvm_run_ptr`
    /// is not large enough to contain the `sync_regs` structure.
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
    ...
    /// unsafe { vcpu.sync_regs_mut().regs.rax = 0xdeadbeef; }
    /// vcpu.set_sync_dirty_reg(SyncReg::Register);
    /// vcpu.run();
    /// ```
    /// # Safety
    ///
    /// This will give a &mut to out of bounds memory if the allocation for `kvm_run_ptr`
    /// is not large enough to contain the `sync_regs` structure.
    #[cfg(any(target_arch = "x86", target_arch = "x86_64"))]
...
mod tests {
    ...
    fn test_sync_regs_with_run() {
        ...
        // SAFETY: vcpu created with `create_vcpu`, so sync_regs is in bounds
        let mut sync_regs = unsafe { vcpu.sync_regs_mut() };
        ...
        // SAFETY: vcpu created with `create_vcpu`, so sync_regs is in bounds
        let sync_regs = unsafe { vcpu.sync_regs() };
